...
export const StartRecording = (onScourses: {
    ...
    // 250ms timeslice: smaller chunks stay well under socket.io's 1MB message
    // limit and are easier to reassemble. 1000ms chunks on HD recordings can
    // silently exceed the limit and get dropped, truncating the video.
...
export const onDataAvailable = async (e: BlobEvent) => {
    // Blobs cannot be serialized by socket.io — convert to ArrayBuffer first
    // so actual raw binary bytes are transmitted over the wire.
